...
for(var i = 0; i < hot_city.length; i++){
    let ii = i;
    hot_city[ii].onclick = function (){
        var fir = document.getElementById('fir');
        var sec = document.getElementById('sec');
        var x = hot_city[ii].getAttribute('data-city');
        var mypro = hot_city[ii].getAttribute('data-province');
        var y = '';
        var proci = {
            pro : hot_city[ii].getAttribute('data-province'),
            city : hot_city[ii].getAttribute('data-city')
        };
        var prociv = JSON.stringify(proci);
        setHistory(prociv);
        if(x == mypro){
            fir.innerHTML = mypro;
            fir.getAttribute('pro') = mypro;
            sec.innerHTML = '';
        }else{
            fir.innerHTML = mypro;
            sec.innerHTML = x;
        }
        fir.setAttribute('pro',mypro);
        jsonp ({
        ...
        if (str != undefined) {
            ...
            for (var i = 0; i < strArr.length; i++) {
                let ii = i;
                var trans = JSON.parse(strArr[ii]);
                if(trans.county == undefined){
                    htmlTxt1 += '<li class="opts1" data-province="' + trans.pro + '"' + 'data-city="' + trans.city  +'"><span>' + trans.city +'</span></li>';

                }else{
                    htmlTxt1 += '<li class="opts1" data-province="' + trans.pro + '"' + 'data-city="' + trans.city  + '"' + 'data-county ="' + trans.county + '"' + '><span>' + trans.county +'</span></li>';
                }
            }
            document.getElementsByClassName("history")[0].innerHTML = htmlTxt1;
        }
